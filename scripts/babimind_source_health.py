#!/usr/bin/env python3
"""Health-check BabiMind external source endpoints.

No credentials are required for the public endpoints in the default catalog.
Failures are recorded as source-quality signals; the checker itself exits 0
unless the catalog is malformed.
"""
...
def check(url: str, timeout: int = 15) -> dict:
    ...
    headers = {"User-Agent": "BabiMind-SourceHealth/1.0 (+https://github.com/babakbadel/Tahlil)"}
    try:
...
def main() -> int:
    ...
    summary = {"checked": len(results), "ok": ok, "unavailable": unavailable, "error": errors}
    payload = {"model": "BabiMind", "checked_at": now, "summary": summary, "sources": results}

    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    REPORT_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = ["# BabiMind Source Health", "", f"Checked: `{now}`", "", f"**OK:** {ok}  |  **Unavailable:** {unavailable}  |  **Error:** {errors}", "", "| Source | Type | Status | HTTP | Latency ms |", "|---|---|---|---:|---:|"]
    for r in results:
        lines.append(f"| {r['name']} | {r['type']} | {r['status']} | {r.get('http_status','-')} | {r.get('latency_ms','-')} |")
    lines += ["", "Health status is operational metadata, not evidence that a source is correct or suitable as Ground Truth."]
