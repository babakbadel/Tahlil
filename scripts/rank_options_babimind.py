...
def jalali_to_gregorian(jy,jm,jd):
    jy-=979; jm-=1; jd-=1
    j=365*jy+jy//33*8+((jy%33)+3)//4+(31*jm if jm<6 else 186+30*(jm-6))+jd
    g=j+79; gy=1600+400*(g//146097); g%=146097; leap=True
    if g>=36525:
...
def implied_vol(S,K,T,P,typ):
    ...
    if P<intrinsic*0.999:return None
    lo,hi=1e-5,5.0
    if bs_price(S,K,T,hi,typ)<P:return None
    for _ in range(80):
        mid=(lo+hi)/2; q=bs_price(S,K,T,mid,typ)
        if q<P:lo=mid
        else:hi=mid
    return (lo+hi)/2
def greeks(S,K,T,sig,typ,r=R):
    ...
    d1=(math.log(S/K)+(r+.5*sig*sig)*T)/(sig*math.sqrt(T));d2=d1-sig*math.sqrt(T)
    gamma=pdf(d1)/(S*sig*math.sqrt(T)); vega=S*pdf(d1)*math.sqrt(T)/100
...
def infer_spots(rows):
    groups={}
    for r in rows:groups.setdefault((r['underlying'],r.get('expiry_key')),[]).append(r)
    spots={}
    for key,rs in groups.items():
        cs={r['strike']:r['price'] for r in rs if r['option_type']=='CALL' and r['price'] is not None}
        ps={r['strike']:r['price'] for r in rs if r['option_type']=='PUT' and r['price'] is not None}
        vals=[]
        T=max((r['days_to_expiry'] or 0)/365,0) for r in rs)
        for k,c in cs.items():
            if k in ps:
                # C-P = S-Ke^-rT => S=C-P+Ke^-rT
                vals.append(c-ps[k]+k*math.exp(-R*T))
        if vals:
            vals.sort();spots[key]=vals[len(vals)//2]
    return spots
def score(r):
    s=35
    ...
    # reward moderate time value relative to spot, penalize obvious extreme OTM
...
def main():
    ...
    for r in rows:
        ...
        if S is not None:
            intrinsic=max(0,S-K) if r['option_type']=='CALL' else max(0,K-S)
            r['intrinsic_value']=round(intrinsic,4);r['time_value']=round(P-intrinsic,4)
            r['breakeven']=round(K+P,4) if r['option_type']=='CALL' else round(K-P,4)
        else:r['intrinsic_value']=r['time_value']=r['breakeven']=None
        r.pop('expiry_key')
        r['babimind_score']=score(r);r['decision']='BUY' if r['babimind_score']>=80 else 'HOLD' if r['babimind_score']>=60 else 'AVOID'
    best={}
    for r in rows:
    ...
    generated=datetime.now(timezone.utc).isoformat();OUT_JSON.parent.mkdir(parents=True,exist_ok=True)
    OUT_JSON.write_text(json.dumps({'generated_at_utc':generated,'record_count':len(rows),'records':rows},ensure_ascii=False,indent=2),encoding='utf-8')
    ...
    for r in rows:
        ...
        lines.append(f"| {r['rank']} | {r['underlying']} | {r.get('symbol') or '-'} | {r['option_type']} | {r['babimind_score']:.1f} | {r['decision']} | {f(r['strike'])} | {f(r['price'])} | {f(r['iv'])} | {f(r['delta'])} | {f(r['gamma'])} | {f(r['theta'])} | {f(r['vega'])} | {f(r['days_to_expiry'])} | {f(r['breakeven'])} |")
